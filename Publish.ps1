...
if ($branch -ne 'master') {
	Write-Error "Not on the default branch. Current branch: $branch"
	exit 1
}
...
if ($behind -gt 0) {
	Write-Error "Local branch is behind origin/master by $behind commit(s)."
	exit 1
}
...
$project = Join-Path $PSScriptRoot 'Cisco.DnaCenter.Api\Cisco.DnaCenter.Api.csproj'
